...
namespace netcore_client
{
    ...
    public delegate void THostShowMessage([MarshalAs(UnmanagedType.LPStr)]string msg);  
    public class Class1
    {
        // delphi側から呼び出される関数。例として呼び出し元のパスと、コールバック関数ポインタが渡される
        public static void Entry([MarshalAs(UnmanagedType.LPStr)] string AppPath, IntPtr funcptr)
        {
